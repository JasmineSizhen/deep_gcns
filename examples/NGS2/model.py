...
#             raise Exception('Unknown block Type')
    ...
    def forward(self, input_batch):
        ...
        edge_weight = input_batch.edge_weight
        batch = input_batch.batch

        h = self.atom_encoder(x) + self.pos_encoder(pos)
        ...
        if self.block == 'res+':
            h = self.gcns[0](h, edge_index, edge_emb, edge_weight)
            for layer in range(1, self.num_layers):
                ...
                h = self.gcns[layer](h2, edge_index, edge_emb, edge_weight) + h
